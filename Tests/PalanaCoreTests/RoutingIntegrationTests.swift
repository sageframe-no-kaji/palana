...
// other one. Push and pull copies run rsyncDirect end to end through
// the router: real rsync on this Mac and real sshd in the container.
// The forwarding probe answers
...
struct RoutingIntegrationTests {
    ...
    @Test("a push copy runs rsyncDirect here and leaves the source standing")
    func pushCopy() async throws {
        ...
        #expect(remaining.sorted() == ["payload.txt", "with space"])
