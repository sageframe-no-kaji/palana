...
The Workbench's mutating proof — the tool the overview's ho-10 always named, now standing on the boundary ho-10 built. Dataset create, destroy, and rename; snapshot take, roll back, and destroy; mountpoint set, clear, and get; pool health and vdevs from `zpool status`. Every mutating verb composes a `PlanRequest` and rides the read-plan-then-Enter gate the mutation seam named — the Surface never composes a mutating command, the Plan Engine classifies it, exactly as file operations do. This is where the seam meets a real mutating consumer and either holds or shows its gaps.

**Depends on:** ho-10 (the protocol and the mutation seam), ho-06.2 (the zfs engine), ho-08 (the plan-and-enact gate)

**What's in scope:**
- Dataset CRUD, snapshots, mountpoint management, pool visualization — mutating verbs routed through the Plan Engine, never composed by the Surface
- Mountpoint set/clear/get (the ho-9.3 sealed followup) lands here, not in core
- The `zpool status` drives fact, parsed, if the host map earns it (the ho-9.3 / ho-10 followup)
- The fuller Workbench surface, if the tool needs chrome beyond a button (the ho-10 deferred decision)
...
**Depends on:** ho-10 (the Workbench), ho-02 (the Conduit), ho-08 (the terminal surface it grows from)

**What's in scope:**
- An interactive shell per host — type into it, run commands, read the live output
- Terminal emulation and PTY handling over the Conduit, per the userland the Field already probes

**What "done" means:**
- The operator types into a per-host shell and it behaves like a terminal — cursor, control sequences, live output — over the same door every read and transfer uses

**What's out of scope:** Tabs and multiplexing — one session per host. Local shell (the reads tool already reaches `local`; a full local shell is the same work pointed at `LocalConduit`).
...
├── ho-9.3 (Mounts + Host Map) · done  ho-9.4 (Favorites) · done
└── ho-9.5 … ho-9.10 (Onboarding, Drag-drop, Polish,
...
- **Checkpoint 3 ANSWERED 2026-07-04, the practitioner's word: "no. ready to advance?" — no reordering, the recommendation stands.** 9.1 Rename+Create → 9.2 Settings → 9.3 Mounts and the Host Map, then ho-10 the Workbench; 9.4 through 9.8 land between the Workbench and the ship ho. Phase 3.5 (the insertion run) opens with ho-9.1.
- **Slate amendment from ho-9.1's hands, 2026-07-05 — two hos queued.** His three findings: an edited file reopened empty (every open fetched a copy into one shared temp path — the second open destroyed the edited copy; errata in the tree: local opens in place, remote opens get a fresh directory per open), create-over-existing refused illegibly (guards now speak: `refused: <path> exists`), and copy silently overwriting a same-named destination. Queued: **ho-9.9 Collision Facts** — the plan names what it will overwrite before Enter arms, a gathered fact line, never an are-you-sure dialog. **ho-9.10 Remote Round-Trip Editing** — watch the fetched copy, compose the upload as a plan when it changes; until it lands, remote edits stay stranded honestly in per-open directories. Create keeps its refusal — creating over a file is truncation wearing creation's name; `r` then `a` says what it means.
...
- **ho-9.4 closed 2026-07-08 — favorites, and the practitioner has driven the column he asked for.** A favorite is a host, a path, and a scope; the truth lives in `PalanaCore`—`Favorite`, `FavoritesList` (the add/remove/toggle/promote logic), `FavoritesStore` (silent-fail load, atomic save into `favorites.json`), and `FavoritesOutline` (grouping and the keyboard row order)—all tested, with `FavoritesModel` a thin `@Observable` shell that observes and persists. The v1 surface the Think phase named—a star in each address bar, a favorites section in the host menu—shipped first; the hands session then grew what the ho anticipated: a floating column on the host-map lineage, opened with `*` or a titlebar star, organized by machine with expanders, global on top, keyboard-navigable like a single pane of the main program (arrows and hjkl, dig and climb, Enter jumps, shift-arrows promote and demote), unstar and promote/demote inline, a one-level-plus undo. The jump destination converged through his hands into a three-arrow selector—left, both, right—defaulting to the pane he was on, one mechanism for mouse and keyboard: "a hot fucking solution." **Review caught the one slip—the surface task, walled off from importing the executable target, wrote reference tests against a copy of the model's logic; the fix relocated that logic into the core's `FavoritesList` and tested it for real.** Platform truth banked: `charactersIgnoringModifiers` applies Shift (so `cmd-shift-8` arrives as `cmd-shift-*`) and drops it for arrow keys (so the panel reads the raw event for shift-arrows). Two agent tasks on claude-sonnet-4-6 plus three hands rounds, reviewed at the top throughout. Deferred to ho-9.8: a "starred" column in the column picker. 508 tests, 85 suites, CI green. **Next: ho-9.5 Host Onboarding—the guided add-a-host flow that writes the `Host` block into `~/.ssh/config` and walks key setup; it mutates the operator's ssh surface, so it opens with a full Think phase.**
- **THE HO-9 REMAINDER EXECUTED 2026-07-10 — five hos authored and built in one autonomous push (his “BIG PUSH”), every AT on claude-sonnet-4-6, reviewed at the top, CI green throughout; close entries follow his hands.** The safety teeth first: **ho-9.9 Collision Facts** — a `CollisionReport` on every destination-ful plan, gathered fresh through the panes’ own listing, rendered in alarm under the size line (replaces, merges into, kind clash — and “destination unread” when the gather fails; silence only when gathered-and-clean). Review caught the report keyed on classification instead of the destination — the mv-based move would have stayed unnamed. **ho-9.10 Remote Round-Trip Editing** — every remote open registers a watch (dual DispatchSource: the directory fd survives atomic-replace saves, the file fd catches in-place writes); a debounced save summons the panel with the upload plan, its collision line naming the replace, a changed-since-fetch note naming a remote that moved; a busy panel is never evicted, Esc declines and the watch survives, the baseline refreshes after a send. **ho-9.6 Drag-and-Drop** — rows drag the selection between panes (the Table moved to its rows-builder form, behavior held), drops compose copy (option: move) through the standing gather, Finder URLs resolve through the local listing, self-drops refuse, the wash marks a valid hover — nothing enacts without Enter. **ho-9.8 Columns** — created/changed on `FileEntry` at sealed per-flavor fidelity (BSD both, GNU changed-only, BusyBox neither; corpora re-recorded live, recorder first), six new columns behind the platform’s header right-click, visibility persisted to `columns.json` (the customization value refused Codable — widths live per-process, the named escape hatch), permissions/owner/group finally on screen, ★ marking favorited directories (display + toggle — the header cannot emit a starred comparator; boundary in the doc). **ho-9.7 Design Polish** — the ledger consolidated (the early-landed fragments named), go-again keys as key-cap chips, a terminal popout on each pane footer, the pruning audit found the palette clean (taste-call ledger in the doc), the cmd-swallow debt closed by verified reading, and the NSMenu refit dissolved honestly: no menu in the app carries sequence hints — a finding, not a diff. Also landed en route: **ho-10.1 AT-01** — the ZFS mutation engine, eight verbs composing delegated no-sudo `zfs` commands with visible verifies, the round trip proven live on the fixture pool. 682 tests, 113 suites, CI green through the run. **Next: ONE hands session over the five (the overwrite line, the round-trip loop in his editor, the drag, the columns, the chips and the popout) → verdicts fill the close entries → ho-10.1 resumes (AT-02 the tool, AT-03 the surface) → ho-11 → ho-12.**
- **THE HANDS SESSION OVER THE BIG PUSH, 2026-07-10 — he drove all five hos live, the session answered in nine rounds, and a new ho was born and shipped inside it.** His verdicts and what they produced: the collision line ratified with his own wording ("N files already exist at destination"); columns "fixed" after the review-missed comparator gap (the six new columns had shipped without value: keypaths — sortable now, and ★ sorts after all via a routing-token comparator applySort intercepts); drag-and-drop took three attempts — the root cause all along was a bundle-less binary poisoning every custom pasteboard type (UTType(string) returns nil undeclared, so the drop view never accepted our drags); the third attempt moved the payload to public.json with logging on both halves and his verdict came back "its working," diagnostics then stripped. His asks, built same-session: per-pane history (⌘←/⌘→ + header chevrons, PaneHistory pure core, forward clears on new navigation); one-Enter rename and create (the name field IS the confirmation — the double-⏎ ceremony was "a fucking PITA" and the guard already refuses collisions); loud prompts (accent-bordered naming field that says what ⏎ does, and the round-trip upload arrives shouting "⏎ press enter to send it back · esc keeps the edit local"); the auto-send toggle ("send saved edits back automatically" — a remote changed since the fetch ALWAYS asks, toggle or no toggle); the verb chips grown into a permanent rail (dimmed while a plan runs). Then the two grammars. **The language sweep**: six rules now govern every sentence the app speaks (plans in the future, running steps through their commands, finished runs in the past with proof, refusals naming thing and reason, unknowns named unknown, no jargon without a compelling reason) — classification and transport words render plain ("copy to another machine · how: rsync, run from this Mac · using this Mac's ssh keys"), raw values frozen. **And ho-9.11 THE GRAMMAR, proposed, ratified, and shipped in one day**: his "TOO loosey goosey" produced a 119-binding audit, five rules (lowercase = verbs, naming verbs ask once, six surface summons closed-set, ⌘ = the app never files, esc retreats and never destroys), and the two changes the rules forced — d deletes and r renames (yazi's letters restored, the most destructive verb off the easiest key, R and T unbound) and the 8-family on one physical key (8 stars the entry, ⌘8 the directory, * the panel, ⇧⌘8 dead). Every key-naming surface moved in one commit; the ? card now teaches the rules; the README gained the keybindings table. 696 tests, 114 suites, CI green through the day. **Still open for his hands: the round-trip loop end-to-end in his editor, the columns picker feel, the retrained r/d muscle, the wash weight — then the batch close entries for 9.6–9.11.**
- **THE HO-9 SERIES CLOSED 2026-07-10, his word: "thats it. finially. can we fucking close ho 9 now?" — Phase 3.5 complete, seven hos sealed in one sitting.** ho-9.5 (onboarding safety verified by reading, his hands reshaped the form, the visible-close was born here), ho-9.6 (drag works — three attempts to learn that a bundle-less binary cannot declare pasteboard types; public.json carries the payload now), ho-9.7 (the polish ledger consolidated, the chips grown into a permanent verb rail, and the floating keys panel's continuous-scale system executed for its crimes — five fixed sizes on ⌘1–⌘5, one authority, after two crashes and a desynced render), ho-9.8 (nine columns behind the header right-click, the review-missed comparators fixed by his report, ★ sorts through a routing token), ho-9.9 (every destination-ful plan names what it will overwrite, his wording verbatim), ho-9.10 (the hands overruled the ask — "save is save": auto-send is the default, a changed remote always asks, the panel never pops uninvited), and ho-9.11 (the grammar: five rules, d deletes, r renames, the 8-family on one key, the ? card teaches, the README carries the table). En route the language sweep gave every operator-facing sentence a six-rule grammar, and per-pane history landed on ⌘←/⌘→. Debts named at close: a binding-table snapshot test before any future key rename; the light text editor queued as a future Workbench plugin. 696 tests, 114 suites, CI green through the whole day. **Next: ho-10.1 resumes — AT-02 (ZFSMutationTool fills the Workbench seam) and AT-03 (the app mutation path, hands against the fixture pool) — then ho-11 (the terminal), ho-12 (the ship).**
