...
- ho-02 closed 2026-07-03 — the Conduit stands: `run(host, command)` behind the protocol, ControlMaster lifecycle (cold 66.6ms → multiplexed 9.9ms against the container fixture), the error taxonomy typing every fixture-producible failure, RecordedConduit/RecordingConduit as the test seam. Deferred decision 2 resolved: Lima — `make zfs-fixture` re-creates the throwaway pool in 1m05s cached (3m24s first run, image download). CI green with the 90% floor enforced; PalanaCore at 99.4% locally, 98.2% on the runner. Next: **ho-03 — The Field**.
