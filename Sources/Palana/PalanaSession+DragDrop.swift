...
extension PalanaSession {
    ...
    /// panes, runs ``DropDecision/decide(payload:targetHost:targetDirectory:moveHeld:)``,
    ...
    /// ``DropDecision/decideOntoFolder(payload:targetHost:folderPath:folderNameData:moveHeld:)``,
    ...
    /// panel is the gate.
    func handleFinderDrop(urls: [URL], targetPane: PaneModel, moveHeld: Bool) {
        routeFinderDrop(
            urls: urls,
            targetPane: targetPane,
            engine: sessionEngine,
