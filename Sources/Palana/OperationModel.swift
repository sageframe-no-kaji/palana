...
final class OperationModel {
    ...
    // MARK: - Pending ZFS state

    /// The ZFS mutation verb waiting for gather input.
    ///
    /// Non-nil while a ZFS gather is open — guards `commitNaming`'s routing.
    var pendingZFSVerb: WorkbenchVerb?
    /// The tool that owns the pending ZFS verb.
    var pendingZFSTool: ZFSMutationTool?
    /// The host the ZFS mutation targets.
    ...
    func begin(_ operation: PlanOperation, source: PaneModel, destination: PaneModel) {
        ...
        resultName = nil
        // A stale ZFS gather must not misroute this operation's naming.
        clearZFSGatherState()
...
extension OperationModel {
    ...
    func beginNaming(_ operation: PlanOperation, source: PaneModel, labelOverride: String? = nil) {
        ...
        if phase == .naming { reset() }
        guard let sourceHost = source.state.host, source.status == .ready else { return }
        // A stale ZFS gather must not misroute this naming's commit —
        // commitNaming routes to the ZFS path while a pending verb stands.
        clearZFSGatherState()
        if operation == .rename {
    ...
    ///
    /// When a pending ZFS verb is live, delegates to ``commitZFSGather(_:)``
    /// instead of the file-verb path.
    func commitNaming(_ name: String) {
        if pendingZFSVerb != nil {
            commitZFSGather(name)
            return
        }
